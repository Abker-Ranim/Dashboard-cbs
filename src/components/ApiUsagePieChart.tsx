...
export const ApiUsagePieChart: React.FC<ApiUsagePieChartProps> = ({
  data = [
    { name: "getaccount", count: 1250, percentage: 35.2, color: "#3B82F6" },
    { name: "getcustomer", count: 890, percentage: 25.1, color: "#10B981" },
    { name: "createorder", count: 650, percentage: 18.3, color: "#F59E0B" },
    { name: "processpay", count: 420, percentage: 11.8, color: "#EF4444" },
  ],
}) => {
  ...
  useEffect(() => {
    ...
    const centerY = (rect.height / 2) - 20 // Keep chart shifted up by 20px
    const radius = Math.min(centerX, centerY) // Further increase circle size
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const centerX = rect.width / 2
    const centerY = (rect.height / 2) - 20 // Adjust for moved chart
    const radius = Math.min(centerX, centerY) - 20

    const distance = Math.sqrt((x - centerX) ** 2 + (y - centerY) ** 2)

    if (distance <= radius) { // Removed center circle check
